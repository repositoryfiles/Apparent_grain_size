...
- GrainSize_2.py 改良バージョン
  - ディクショナリからリスト表記への変更
  - 試験線を座標値（testline_pts）として求めておき、粒界の検出や、マウスクリックによる粒界位置の調整に用いるように変更（直線の試験線に対応するための前作業）
  - 読み込んだ画像の幅と撮影倍率から、画像の高さを140mmにしたときの倍率を算出し、これら画像の高さと倍率を解析に使用する（直線の試験線に対応するための前作業）
  - 不要な変数を削除
